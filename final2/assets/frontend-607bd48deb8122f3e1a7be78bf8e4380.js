...
(function ($, window, document, undefined) {
  ...
  function setLabelHeight() {
    if (Modernizr.csstransforms && $(window).width() > 600) {
  ...
  function randomTransform() {
    var max = 20,
        randX = (Math.random() * (max/6) * 2) - (max/6) + "deg",
        randY = (Math.random() * max * 2) - max + "deg",
        randZ = (Math.random() * max * 2) - max + "deg";
    $('.main').transition({
      rotateX: randX,
      rotateY: randY,
      rotateZ: randZ,
  ...
  $(function () {
    ...
    //$(document).pjax('a[data-pjax-link]', '[data-pjax-container]');
    //$('img').lazyload({
    //  container: $("#content")
    //});
    ...
    if ($.support.pjax) {
      ...
      $(document).on('pjax:complete', function() {
        $('#content').scrollTop(0);
        $('body').removeClass('loading activeSuperMenu darkenPage');
        $('.menuButton').removeClass('spin');
        resetTransform();
        setLabelHeight();
        //$('img').lazyload({
        //  container: $("#content")
      ...
      $(document).on('click', 'a[data-pjax-link]', function(event) {
        var $that = $(this),
          target = event;
        $('.menuButton').addClass('spin');

        if ($that.hasClass('super')) {
          $that.parent().parent().find('a').removeClass('active');
          $that.addClass('active');
        }
        if ($that.hasClass('filter-tag') === false) {
          $('body').addClass('loading darkenPage');
          randomTransform();
        }
        //setTimeout(function(){
        //  $(body).removeClass('activeSuperMenu');
        //},600);
        var container = $('[data-pjax-container]');
        $.pjax.click(target, {container: container});
        event.preventDefault();
      });
    ...
    $(document).on('click', '.menuButton', function(event){
      if ($('body').hasClass('activeSuperMenu')) {
        $('body').removeClass('activeSuperMenu darkenPage');
        resetTransform();
      } else {
        $('body').addClass('activeSuperMenu darkenPage');
        randomTransform();
      }
      false
    });
    ...
    //    $('body').removeClass('activeSuperMenu loading');
    //    $('.menuButton').removeClass('spin');
    //  },2000);
    //});
    ...
    $('#content').on('scroll', function() {
      ...
      if (scrolled > 0) {
        window.currentScroll = scrolled;
      }
    });

    $(document).on('webkitfullscreenchange mozfullscreenchange fullscreenchange', function() {
      $('#content').scrollTop(window.currentScroll);
    });
